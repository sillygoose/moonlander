...
- (NSArray *)buildDetailedLunarSurface
{
    ...
    if (TEMP < 0)
        TEMP = 0;
    else if (TEMP > self.bounds.size.height)
    ...
    for (short i = terrainIndex; lineSegments < 225; i += nextIndex) {
        ...
        while (k-- > 0) {
            ...
            if (--DRAWTY < 0) {
                ...
                NSDictionary *lineType = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:DTYPE], @"type", [NSNumber numberWithFloat:2.5f], @"width", nil];
